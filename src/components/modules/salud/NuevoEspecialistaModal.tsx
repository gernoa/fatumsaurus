...
export function NuevoEspecialistaModal({ onSaved, onClose }: Props) {
  ...
  async function handleSave() {
    ...
    if (!tipoFinal)     { setErr('Elige o escribe el tipo de especialista'); return }
    ...
    try {
      const base = {
        ...
        sesiones_contratadas: null as number | null,
        fecha_pago:           null as string | null,
        precio_sesion:        null as number | null,
      }
      if (modalidad === 'bono') {
        base.precio_total         = parseFloat(precio.replace(',', '.')) || 0
        base.sesiones_contratadas = parseInt(sesiones) || 10
        base.fecha_pago           = fechaPago || null
      } else {
        base.precio_sesion = parseFloat(precioSes.replace(',', '.')) || 0
      }
      const nuevo = await createEspecialista(base)
      onSaved(nuevo)
      toast.success('Especialista añadido')
      onClose()
    } catch {
    ...
    <>
      ...
      <div className="fixed inset-x-4 bottom-0 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-md z-[--z-modal] bg-card rounded-t-[20px] sm:rounded-[20px] shadow-[0_8px_40px_rgba(0,18,25,0.2)] flex flex-col max-h-[90dvh]">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* Duración por sesión */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Duración estándar por sesión (minutos)</label>
            <input
              type="number"
              value={duracion}
          ...
          {modalidad === 'bono' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Nº sesiones contratadas</label>
                  <input
                    type="number"
                    value={sesiones}
          ...
          {/* Forma de pago — solo si hay precio */}
          {((modalidad === 'bono' && precio) || (modalidad === 'por_sesion' && precioSes)) && (
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">
                {modalidad === 'bono' ? 'Pago del bono con cargo a' : 'Cada sesión se pagará con'}
              </label>
              <div className="flex gap-2">
                {([
